...
def generate_and_publish_files(
    ...
    print("="*60)
    print(" PRODUCER - GENERATING AND PUBLISHING FILES (TASK 11)")
    print("="*60)
    
    queue_mgr = EnhancedQueueManager()
    ...
    try:
        ...
        for i in range(num_csv):
            ...
            if filepath.exists():
                ...
                queue_mgr.publish_to_main_queue(filepath)
                time.sleep(0.1)
        ...
        print(f" Main Queue: {queue_mgr.main_queue}")
        
        main_size = queue_mgr.get_queue_size(queue_mgr.main_queue)
        broken_size = queue_mgr.get_queue_size(queue_mgr.broken_queue)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Generate files and publish to main queue (Task 11)'
    )
    ...
    parser.add_argument('--with-errors', action='store_true', 
                        help='Include corrupted files to test broken queue')
